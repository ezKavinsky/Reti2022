...
public class BetServer {
    public HashMap<Integer, Scommessa> scommesse;
        ...
        public void run(){
            while(accept) {
                try {
                    ...
                    PrintWriter out = new PrintWriter(k.getOutputStream(), true);        //riceve la scommessa (nel formato "numcavallo puntata") e la inserisce nella lista scommesse
    ...
    public void comunicaVincitori(LinkedList<Scommessa> vincitori, InetAddress ind, int port){
        ...
        try{
            ...
            String m = "";
            int quota = 12;     //il moltiplicatore della puntata vincente
            while(it.hasNext()){
                Scommessa s = it.next();
