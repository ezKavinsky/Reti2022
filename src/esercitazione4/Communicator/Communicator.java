...
public class Communicator {
    ...
    static void sendMcastDatagram(){
        try{
            while(true){
                ...
                DatagramPacket packet = new DatagramPacket(buf, buf.length, mcastAddress, 2000);
    ...
    static class MulticastListener extends Thread{
        ...
        int tcpPort;        //è la porta su cui il Communicator è in ascolto per ricevere msg TCP (questo valore verrà inserito nel msg di risposta)
        ...
        public void run(){
            try{
                ...
                while(true){
                    //Riceve datagramma multicast ed estrae da esso l'indirizzo del mittente (remAddress) e la porta del modulo server del mittente (remTCPPort),
                    ...
                    System.out.println("ML> Cotenuto del datagram: " + remTCPPort);
                    //Invio messaggio in TCP solo se il datagramma multicast era stato inviato da un altro Communicator (non da se stesso);
    ...
    static class SocketListener extends Thread{
        ...
        public SocketListener(int port){
            this.tcpPort = port;
            System.out.println("SL> Porta TCP locale = " + tcpPort);
        }//constructor
        ...
        public void run(){
            try{
                ...
                while(true){
                    //Ricevo messaggio di risposta via TCP (è la risposta di un altro Communicator al msg che è stato inviato in multicast)
                    Socket sock = sListener.accept();
                    InetAddress remAddress = sock.getInetAddress();
                    BufferedReader in = new BufferedReader(new InputStreamReader(sock.getInputStream()));
                    String line = in.readLine();
                    //Il msg (risposta) contiene la porta TCP del Communicator che ha risposto e che viene usata per ricevere (a sua volta) msg TCP (remTCPPort)
